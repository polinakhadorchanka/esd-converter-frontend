...
const DocToEsd = () => {
  const { addToDocument, addToSignatures, deleteDocument, deleteSignature, convertDocsToESD, clearError } =
    useActions();
  ...
  const onConvertClick = () => {
    clearError();
    mainDocument && signatures.length > 0 && convertDocsToESD({ mainDocument, signatures });
  };
  ...
  return (
    <MainLayout>
      ...
      <div className={'mt-4 flex flex-row justify-center items-center'}>
        {!isProcessing && (
          <Button isDisabled={!mainDocument || signatures.length === 0} onClick={onConvertClick}>
            Convert
          </Button>
        )}
